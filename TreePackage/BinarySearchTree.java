                ...
                implements SearchTreeInterface<T>
{
    ...
    private T addEntry(T newEntry)
    {
        ...
        BinaryNode<T> leastNode = getRootNode();
        BinaryNode<T> mostNode = getRootNode();
        ...
        while (!found)
        {
            ...
            else if (comparison < 0)
            {
                if (currentNode.hasLeftChild()){
                    if(currentNode.getLeftChild().getData().compareTo(leastNode.getData()) < 0
                        || currentNode.getLeftChild().getData().compareTo(newEntry) < 0)
                        leastNode = currentNode.getLeftChild();
                    if(currentNode.getLeftChild().getData().compareTo(mostNode.getData()) > 0
                        || currentNode.getLeftChild().getData().compareTo(newEntry) > 0)
                        mostNode = currentNode.getLeftChild();
                ...
                } else
                {
                    ...
                    currentNode.getLeftChild().setThread(currentNode);
                    if(currentNode.getLeftChild().getData().compareTo(leastNode.getData()) > 0)
                        leastNode.setThread(currentNode.getLeftChild());
            ...
            else
            {
                ...
                if (currentNode.hasRightChild()){
                    if(currentNode.getRightChild().getData().compareTo(leastNode.getData()) < 0
                        || currentNode.getRightChild().getData().compareTo(newEntry) < 0)
                        leastNode = currentNode.getRightChild();
                    if(currentNode.getRightChild().getData().compareTo(mostNode.getData()) > 0
                        || currentNode.getRightChild().getData().compareTo(newEntry) > 0)
                        mostNode = currentNode.getRightChild();
                ...
                } else
                {
                    ...
                    currentNode.setThread(currentNode.getRightChild());
                    if(currentNode.getRightChild().getData().compareTo(mostNode.getData()) < 0)
                        currentNode.getRightChild().setThread(mostNode);
    ...
    private BinaryNode<T> getInorderPredecessor(BinaryNode<T> node) {
        BinaryNode<T> curr = node;
        if(curr != null){
            if (node.hasLeftChild()) {
                curr = node.getLeftChild();
                ...
                while (curr != null && curr.getLeftChild() == child) {
                    child = curr;
                    curr = curr.getParent();
                }
            }
        }
        return curr; // may be null if node is the inorder minimum
    }
                            ...
                            BinaryNode<T> parentNode)
    {
        ...
        {
            BinaryNode<T> pred = getInorderPredecessor(nodeToRemove);
            if(pred != null)
                pred.setThread(nodeToRemove.getThread());
            setRootNode(childNode);
        }
        else if (parentNode.getLeftChild() == nodeToRemove)
        {
            if(childNode != null && childNode.getThread() == nodeToRemove)
                childNode.setThread(parentNode);
            else if(childNode != null){
                BinaryNode<T> curr = childNode;
                while(curr.hasRightChild())
                    curr = curr.getRightChild();
                curr.setThread(parentNode);
            }
            BinaryNode<T> pred = getInorderPredecessor(nodeToRemove);
            if(pred != null && pred.getThread() == nodeToRemove)
                pred.setThread(nodeToRemove.getThread());
        ...
        {
            if(childNode != null){
                childNode.setThread(nodeToRemove.getThread());
            }
            parentNode.setThread(nodeToRemove.getThread()); 
